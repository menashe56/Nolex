...
const ChatScreen = ({ navigation, route}) => {
    ...
    const sendMessage = () => {
        ...
        firebase.firestore().collection("chats").doc(route.params.id).collection('messages').add({
    ...
    useLayoutEffect(() => {
        const unSubscribe = firebase.firestore().collection("chats").doc(route.params.id).collection('messages').orderBy('timestamp','desc').onSnapshot(onSnapshot => setMessage(onSnapshot.docs.map(doc => ({
            id: doc.id,
            data: doc.data()
        }))))
        return unSubscribe;
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "white"}}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS ==="ios" ? "padding" : "height"} style={styles.container} keyboardVerticalOffset={90}>
                ...
                {messages.map(({id, data}) => (
                    data.email === firebase.auth().currentUser.email ? (
                        <View key={id} style={styles.reciver}>
                            <Avatar postition="absolute" rounded bottom={-15} right={-5} size={30} source={{uri: data.photoURL,}}/>
            ...
            <View style={styles.footer}>
                <TextInput placeholder='Signsl Massage' style={styles.textInput} value={input} onChangeText={(text) => setInput(text)} onSubmitEditing={sendMessage}/>
